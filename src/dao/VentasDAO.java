...
public class VentasDAO {
            ...
            if (metodos.isEmpty()) {
                ...
                try {
                    ...
                    try (PreparedStatement psInsert = con.prepareStatement(sqlInsert,
                        ...
                        for (String metodo : defaults) {
                            ...
                            try (ResultSet rsKeys = psInsert.getGeneratedKeys()) {
                                if (rsKeys.next())
                                    metodos.put(rsKeys.getInt(1), metodo);
                            }
                        }
                    }
                    ...
                    try (PreparedStatement psInsertManual = con.prepareStatement(sqlInsertManual)) {
                        int id = 1;
                        for (String metodo : defaults) {
                            psInsertManual.setInt(1, id);
                            psInsertManual.setString(2, metodo);
    ...
    public boolean empresaTieneFacturacionHabilitada(int idEmpresa) {
        ...
        try (Connection con = factory.getConexion(); PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next())
                    return rs.getBoolean("habilitar_facturacion_empresa");
            }
        } catch (SQLException e) {
            System.err.println("Error verificando facturación: " + e.getMessage());
        }
        return false;
    }
    ...
    public boolean procesarVentaCompleta(int idCliente, int idUsuario, int idMetodoPago, double subtotal,
            double impuesto, double total, String referenciaPago, String bancoPago, List<Object[]> detalles) {
        ...
        try {
            ...
            try (PreparedStatement psVenta = con.prepareStatement(sqlVenta, Statement.RETURN_GENERATED_KEYS)) {
                ...
                if (referenciaPago == null || referenciaPago.trim().isEmpty())
                    psVenta.setNull(7, java.sql.Types.VARCHAR);
                else
                    psVenta.setString(7, referenciaPago.trim());

                if (bancoPago == null || bancoPago.trim().equals("Seleccione Banco..."))
                    psVenta.setNull(8, java.sql.Types.VARCHAR);
                else
                    psVenta.setString(8, bancoPago.trim());
            ...
            try (PreparedStatement psDetalle = con.prepareStatement(sqlDetalle);
                ...
                for (Object[] fila : detalles) {
                    ...
                    int diasGarantia = (fila.length > 6 && fila[6] != null) ? (int) fila[6] : 0; // Atrapamos la
                                                                                                 // garantía
                    ...
                    try (ResultSet rsStock = psStockActual.executeQuery()) {
                        if (rsStock.next())
                            stockActual = rsStock.getInt("stock_producto");
                    }
            ...
            System.err.println("Error procesando venta y kardex: " + e.getMessage());
            return false;
        } finally {
            if (con != null)
                try {
