...
public class CatalogosDAO {
    ...
    public Map<Integer, String> listarCategorias() {
        ...
        try (Connection con = factory.getConexion();
            ...
            while (rs.next())
                mapa.put(rs.getInt("id_categoria"), rs.getString("nombre_categoria"));
    ...
    public String[] obtenerDatosCategoria(int id) {
        ...
        try (Connection con = factory.getConexion(); PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next())
                return new String[] { rs.getString("descripcion_categoria"), rs.getString("dias_garantias") };
        } catch (SQLException e) {
        }
        return new String[] { "", "0" };
    }
    ...
    public Map<Integer, String> listarProveedores() {
        ...
        try (Connection con = factory.getConexion();
                ...
                ResultSet rs = ps.executeQuery()) {
            while (rs.next())
                mapa.put(rs.getInt("id_proveedor"), rs.getString("nombre_proveedor"));
        } catch (SQLException e) {
    ...
    public String[] obtenerDatosProveedor(int id) {
        ...
        try (Connection con = factory.getConexion(); PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next())
                return new String[] { rs.getString("nombre_encargado_proveedor"), rs.getString("telefono_proveedor"),
                        rs.getString("direccion_proveedor"), rs.getString("tipo_repuestos_proveedor") };
        } catch (SQLException e) {
        }
        return new String[] { "", "", "", "" };
    }

    public boolean actualizarProveedor(int id, String nombre, String encargado, String tel, String dir,
            String repuestos) {
    ...
    public Map<Integer, String> listarUbicaciones() {
        ...
        try (Connection con = factory.getConexion();
                ...
                ResultSet rs = ps.executeQuery()) {
            while (rs.next())
                mapa.put(rs.getInt("id_ubicacion"), rs.getString("nombre_ubicacion"));
        } catch (SQLException e) {
            System.err.println("Error Ubicaciones: " + e.getMessage());
        }
        return mapa;
    }
