...
public class DialogoMovimientoKardex extends JDialog {
    ...
    private void iniciarDiseno() {
        ...
        rbAgregar.setForeground(new Color(39, 174, 96));
        rbAgregar.setBackground(new Color(255, 255, 255)); // Verde Menta
        rbDisminuir = new JRadioButton("Salida (-)");
        rbDisminuir.setForeground(new Color(227, 0, 15));
        rbDisminuir.setBackground(new Color(255, 255, 255)); // Rojo Logo
        ...
        pnlRadios.add(rbAgregar);
        pnlRadios.add(rbDisminuir);

        gbc.gridy = 0;
        pnlForm.add(lblTipo, gbc);
        gbc.gridy = 1;
        pnlForm.add(pnlRadios, gbc);

        // Cantidad
        JLabel lblCantidad = new JLabel("Cantidad:");
        lblCantidad.setForeground(new Color(45, 45, 45)); // Gris Oscuro
        txtCantidad = new JTextField();
        txtCantidad.setBackground(new Color(250, 250, 250));
        txtCantidad.setForeground(new Color(45, 45, 45));
        txtCantidad.setCaretColor(new Color(45, 45, 45));
        txtCantidad
        ...
        gbc.gridy = 2;
        pnlForm.add(lblCantidad, gbc);
        gbc.gridy = 3;
        pnlForm.add(txtCantidad, gbc);

        // Observación
        JLabel lblObs = new JLabel("Motivo u Observación:");
        lblObs.setForeground(new Color(45, 45, 45)); // Gris Oscuro
        ...
        scrollObs.setBorder(BorderFactory.createLineBorder(new Color(220, 222, 225)));
        ...
        btnCancelar.addActionListener(e -> this.dispose()); // Gris Suave
    ...
    private void firmarYGuardar() {
        if (txtCantidad.getText().trim().isEmpty() || txtObservacion.getText().trim().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Debe ingresar cantidad y observación.", "Aviso",
        ...
        if (!esEntrada && cantidad > productoActual.getStockProducto()) {
            JOptionPane.showMessageDialog(this, "Stock insuficiente para esta salida.", "Error",
        ...
        int opcion = JOptionPane.showConfirmDialog(this, msg, "Firma de Autorización", JOptionPane.OK_CANCEL_OPTION,
            ...
            if (idAutorizador > 0) {
                if (pass.equals(String.valueOf(idAutorizador))) {
                    JOptionPane.showMessageDialog(this,
                            "Acceso Denegado: Por políticas de seguridad de Nexar, no se permite autorizar transacciones con contraseñas por defecto. Por favor, actualice su contraseña.",
                            "Seguridad Nexar", JOptionPane.ERROR_MESSAGE);
                    return;
                }

                if (dao.registrarMovimiento(productoActual.getIdProducto(), esEntrada ? "Entrada" : "Salida", cantidad,
                        txtObservacion.getText().trim(), idAutorizador)) {
                    JOptionPane.showMessageDialog(this, "Movimiento registrado y firmado exitosamente.", "Éxito",
                            JOptionPane.INFORMATION_MESSAGE);

                    padreKardex.cargarHistorial();
                    int nuevoStock = esEntrada ? (productoActual.getStockProducto() + cantidad)
                            : (productoActual.getStockProducto() - cantidad);
                    padreKardex.actualizarStockVisual(nuevoStock);

                    this.dispose();
                } else {
                    JOptionPane.showMessageDialog(this, "Error de base de datos al guardar movimiento.",
                            "Error Crítico", JOptionPane.ERROR_MESSAGE);
                }
            } else {
                JOptionPane.showMessageDialog(this, "Contraseña incorrecta o usuario inactivo.", "Firma Rechazada",
                        JOptionPane.ERROR_MESSAGE);
            }
        }
    }
