...
public class PanelApartados extends JPanel {
    ...
    private void filtrarApartados() {
        ...
        for (Apartado a : apartados) {
            ...
            String clie = (a.getNombreCliente() + " " + (a.getApellidoCliente() != null ? a.getApellidoCliente() : ""))
                    .toLowerCase();
            String dni = a.getIdentidadCliente() != null
                    ? a.getIdentidadCliente().toLowerCase().replace("-", "").replace(" ", "")
                    : "";
    ...
    private void registrarAbonoRapido() {
        ...
        if (selectedRow < 0) {
            JOptionPane.showMessageDialog(this, "Seleccione un apartado de la lista.", "Aviso",
        ...
        if (ap.getEstadoApartado().equalsIgnoreCase("Cancelado")
                || ap.getEstadoApartado().equalsIgnoreCase("Entregado")) {
            JOptionPane.showMessageDialog(this,
                    "No se pueden aplicar abonos a apartados en estado: " + ap.getEstadoApartado(), "Aviso",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        if (CCActiva == null) {
            JOptionPane.showMessageDialog(this,
                    "Operación denegada: El turno de caja no está abierto. Abra la caja antes de registrar cobros.",
    ...
    private void entregarArticulos() {
        ...
        if (ap.getSaldoPendiente() > 0.05) {
            JOptionPane.showMessageDialog(this,
                    "No se puede entregar la mercancía de un apartado con saldo pendiente (L "
                            + String.format("%,.2f", ap.getSaldoPendiente()) + ").",
                    "Acceso Denegado", JOptionPane.ERROR_MESSAGE);
            return;
        }

        if (ap.getEstadoApartado().equalsIgnoreCase("Entregado")) {
            JOptionPane.showMessageDialog(this, "El apartado ya fue entregado.", "Aviso",
                    JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        if (ap.getEstadoApartado().equalsIgnoreCase("Cancelado")) {
            JOptionPane.showMessageDialog(this, "El apartado se encuentra cancelado.", "Aviso",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }
    ...
    private void cancelarApartadoCompleto() {
        ...
        if (ap.getEstadoApartado().equalsIgnoreCase("Cancelado")
                || ap.getEstadoApartado().equalsIgnoreCase("Entregado")) {
            JOptionPane.showMessageDialog(this,
                    "No se puede cancelar un apartado con estado: " + ap.getEstadoApartado(), "Aviso",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }

        int opt = JOptionPane.showConfirmDialog(this,
                "Al cancelar, los productos se devolverán al inventario activo. ¿Desea continuar?", "Cancelar Apartado",
                JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
        if (opt == JOptionPane.YES_OPTION) {
            // Password signature
            JPasswordField pfPass = new JPasswordField();
            int opSign = JOptionPane.showConfirmDialog(this,
                    new Object[] { "Ingrese su contraseña para firmar la cancelación:", pfPass }, "Firma Requerida",
            ...
            if (dao.cancelarApartado(id, idUserFirma)) {
                JOptionPane.showMessageDialog(this, "Apartado cancelado exitosamente y stock retornado al inventario.",
                        "Éxito", JOptionPane.INFORMATION_MESSAGE);
                cargarApartados();
            } else {
                JOptionPane.showMessageDialog(this, "Error al registrar cancelación.", "Error",
                        JOptionPane.ERROR_MESSAGE);
            }
        }
    }
