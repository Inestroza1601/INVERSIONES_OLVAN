...
<<<<<<< HEAD
        ...
        JLabel lblSub = new JLabel(
                "<html><body>Hemos enviado un código de 6 dígitos a su correo.<br>Ingrese el código y su nueva contraseña.</body></html>");
...
>>>>>>> origin/parte-muoz
        ...
        txtToken.addKeyListener(new KeyAdapter() {
            public void keyTyped(KeyEvent e) {
                if (txtToken.getText().length() >= 6 || !Character.isDigit(e.getKeyChar()))
                    e.consume();
            }
        });
        ...
        gbc.gridy = 2;
        pnlFondo.add(new JLabel("Código de Seguridad:"), gbc);
        gbc.insets = new Insets(0, 5, 10, 5);
        gbc.gridy = 3;
        pnlFondo.add(txtToken, gbc);

        gbc.insets = new Insets(5, 5, 5, 5);
        gbc.gridy = 4;
        pnlFondo.add(new JLabel("Nueva Contraseña:"), gbc);
        gbc.insets = new Insets(0, 5, 10, 5);
        gbc.gridy = 5;
        pnlFondo.add(txtNuevaClave, gbc);

        gbc.insets = new Insets(5, 5, 5, 5);
        gbc.gridy = 6;
        pnlFondo.add(new JLabel("Confirmar Contraseña:"), gbc);
        gbc.insets = new Insets(0, 5, 15, 5);
        gbc.gridy = 7;
        pnlFondo.add(txtConfirmarClave, gbc);
...
>>>>>>> origin/parte-muoz
    ...
    private void guardarPassword() {
        ...
        if (token.isEmpty() || pass.isEmpty() || conf.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Todos los campos son obligatorios.", "Campos vacíos",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }

        if (!pass.equals(conf)) {
            JOptionPane.showMessageDialog(this, "Las contraseñas no coinciden.", "Error de validación",
        ...
        if (!dao.validarTokenRecuperacion(usuario.getIdUsuario(), token)) {
            JOptionPane.showMessageDialog(this, "El código de seguridad es inválido o ha expirado.",
        ...
        if (dao.actualizarPassword(usuario.getIdUsuario(), pass)) {
            ...
            JOptionPane.showMessageDialog(this, "Ocurrió un error al actualizar la contraseña.", "Error",
                    JOptionPane.ERROR_MESSAGE);
        }
    }
}