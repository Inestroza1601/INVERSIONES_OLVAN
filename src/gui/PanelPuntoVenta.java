...
public class PanelPuntoVenta extends JPanel {
    ...
    public PanelPuntoVenta() {
        if (utilidades.SesionGlobal.getEmpresaActual() != null) {
            facturacionHabilitada = new dao.VentasDAO()
                    .empresaTieneFacturacionHabilitada(utilidades.SesionGlobal.getEmpresaActual().getIdEmpresa());
        }
        iniciarDiseno();
        cargarMetodosPago();
    }
    ...
    private void iniciarDiseno() {
        ...
        pnlClientes.add(lblClie);
        pnlClientes.add(lblClienteSeleccionado);
        ...
        tablaVentas.getColumnModel().getColumn(7).setMinWidth(0);
        tablaVentas.getColumnModel().getColumn(7).setMaxWidth(0);
        tablaVentas.getColumnModel().getColumn(8).setMinWidth(0);
        ...
        tablaVentas.addMouseMotionListener(new java.awt.event.MouseMotionAdapter() {
            ...
            public void mouseMoved(java.awt.event.MouseEvent e) {
                if (tablaVentas.columnAtPoint(e.getPoint()) == 1)
                    tablaVentas.setCursor(new Cursor(Cursor.HAND_CURSOR));
                else
            ...
            public void mouseClicked(java.awt.event.MouseEvent e) {
                ...
                if (f >= 0) {
                    ...
                    if (c == 1 && SwingUtilities.isLeftMouseButton(e)) {
                        mostrarZoomImagen(
                                (String) modeloTablaVentas.getValueAt(tablaVentas.convertRowIndexToModel(f), 7));
                    }
                    if ((SwingUtilities.isLeftMouseButton(e) && e.getClickCount() == 2 && c != 1)
        ...
        pnlControlVenta.setPreferredSize(new Dimension(300, 0));

        JPanel pnlLiquidacion = new JPanel(new GridBagLayout());
        pnlLiquidacion.setBackground(new Color(255, 255, 255));
        pnlLiquidacion.setBorder(
                BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(new Color(220, 222, 225), 1, true),
                        BorderFactory.createEmptyBorder(15, 15, 15, 15)));
        ...
        cmbMetodoPago.setBackground(new Color(255, 255, 255));

        pnlCamposExtraPago = new JPanel(new GridLayout(4, 1, 0, 5));
        pnlCamposExtraPago.setOpaque(false);
        pnlCamposExtraPago.setVisible(false);

        JLabel lblBanco = new JLabel("Banco Emisor:") {
            {
                setForeground(new Color(100, 100, 100));
                setFont(new Font("Segoe UI", Font.BOLD, 12));
            }
        };
        String[] bancosHonduras = { "Seleccione Banco...", "Banco Atlántida", "BAC Credomatic", "Banco Ficohsa",
                "Banpaís", "Banco de Occidente", "Banco Banrural", "Banco Promerica", "Banco LAFISE", "Banco FICENSA",
                "Banco BANHCAFE", "ACH - Transferencia Interbancaria" };
        cmbBanco = new JComboBox<>(bancosHonduras);
        cmbBanco.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        cmbBanco.setBackground(new Color(255, 255, 255));

        lblReferenciaPago = new JLabel("Nº Referencia:") {
            {
                setForeground(new Color(100, 100, 100));
                setFont(new Font("Segoe UI", Font.BOLD, 12));
            }
        ...
        txtReferenciaPago.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        txtReferenciaPago.setBackground(new Color(255, 255, 255));
        txtReferenciaPago.setForeground(new Color(45, 45, 45));
        txtReferenciaPago.setCaretColor(new Color(45, 45, 45));
        txtReferenciaPago.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(new Color(220, 222, 225)), BorderFactory.createEmptyBorder(0, 5, 0, 5)));
        txtReferenciaPago.setPreferredSize(new Dimension(0, 32));

        pnlCamposExtraPago.add(lblBanco);
        pnlCamposExtraPago.add(cmbBanco);
        pnlCamposExtraPago.add(lblReferenciaPago);
        pnlCamposExtraPago.add(txtReferenciaPago);
        ...
        cmbMetodoPago.addActionListener(e -> {
            ...
            if (pagoSeleccionado != null) {
                ...
                if (nombrePago.contains("transferencia") || nombrePago.contains("tarjeta")) {
                    lblReferenciaPago.setText(
                            nombrePago.contains("transferencia") ? "Nº Referencia / ACH:" : "Nº Voucher / Referencia:");
                ...
                pnlLiquidacion.revalidate();
                pnlLiquidacion.repaint();
            }
            recalcularTotales();
        });

        lblSubtotal = new JLabel("Subtotal: L 0.00");
        lblSubtotal.setForeground(new Color(140, 145, 150));
        lblSubtotal.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        lblImpuesto = new JLabel("ISV (15%): L 0.00");
        ...
        gbc.gridy = 4;
        gbc.insets = new Insets(5, 0, 5, 0);
        pnlLiquidacion.add(pnlCamposExtraPago, gbc);
        gbc.gridy = 5;
        gbc.insets = new Insets(15, 0, 0, 0);
        pnlLiquidacion.add(lblSubtotal, gbc);
        gbc.gridy = 6;
        gbc.insets = new Insets(5, 0, 0, 0);
        pnlLiquidacion.add(lblImpuesto, gbc);
        gbc.gridy = 7;
        gbc.insets = new Insets(15, 0, 0, 0);
        pnlLiquidacion.add(new JLabel("TOTAL:") {
            {
    ...
    public void agregarProductoAVenta(Producto p) {
        if (p.getStockProducto() < 1) {
            JOptionPane.showMessageDialog(this, "Stock agotado. No hay unidades disponibles en vitrina.",
        ...
        if (p.isRequiereSerie()) {
            imei = JOptionPane.showInputDialog(this,
                    "Este producto requiere un identificador (IMEI/ServiceTag).\nIngrese el código para: "
                            + p.getNombreProducto(),
                    "Validación de Garantía", JOptionPane.QUESTION_MESSAGE);

            if (imei == null || imei.trim().isEmpty()) {
                JOptionPane.showMessageDialog(this,
                        "Operación cancelada. Debe ingresar el IMEI/Serie para facturar este producto.", "Aviso",
            ...
            for (int i = 0; i < modeloTablaVentas.getRowCount(); i++) {
                ...
                if (valorImeiFila != null && valorImeiFila.toString().equalsIgnoreCase(imei)) {
                    JOptionPane.showMessageDialog(this,
                            "El identificador '" + imei + "' ya está en la lista de compras actual.",
                            "Identificador Duplicado", JOptionPane.ERROR_MESSAGE);
                    return;
                }
            }
            ...
            if (daoVentas.existeIdentificadorVendido(imei)) {
                JOptionPane.showMessageDialog(this, "¡ALERTA! El identificador '" + imei
                        + "' ya se encuentra registrado como VENDIDO en la base de datos.\nVerifique el equipo físico o contacte al administrador.",
                        "Fraude / Error Detectado", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }

        // Si NO requiere serie Y NO TIENE GARANTÍA, agrupamos las cantidades si ya está
        // en la tabla
            ...
            for (int i = 0; i < modeloTablaVentas.getRowCount(); i++) {
                if ((int) modeloTablaVentas.getValueAt(i, 0) == p.getIdProducto()) {
                    ...
                    if (cantActual >= p.getStockProducto()) {
                        JOptionPane.showMessageDialog(this,
                                "Stock insuficiente. Solo hay " + p.getStockProducto() + " unidades en vitrina.",
        ...
        // Si requiere serie, o es un repuesto nuevo, lo agregamos como fila
        // independiente
    ...
    private void buscarProductoPorCodigo(String codigo) {
        ...
        if (p != null)
            agregarProductoAVenta(p);
        else
            JOptionPane.showMessageDialog(this, "Producto no encontrado o inactivo.", "Aviso",
                    JOptionPane.WARNING_MESSAGE);
        txtCodigoBarrasBusqueda.setText("");
        txtCodigoBarrasBusqueda.requestFocus();
    }
    ...
    private void modificarPrecio() {
        ...
        // Le agregamos un filtro que se dispara cada vez que el usuario presiona una
        // tecla
        txtNuevoPrecio.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyTyped(java.awt.event.KeyEvent e) {
                char c = e.getKeyChar();
                // Si la tecla NO es un número, NO es un punto y NO es la tecla de borrar -> la
                // bloqueamos
        ...
        int opcion = JOptionPane.showConfirmDialog(this, new Object[] { "Nuevo Precio Unitario:", txtNuevoPrecio },
                "Modificar Precio", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
    ...
    private void modificarCantidad() {
        ...
        if (modeloTablaVentas.getValueAt(f, 9) != null || (int) modeloTablaVentas.getValueAt(f, 8) > 0) {
            JOptionPane.showMessageDialog(this,
                    "No puede modificar la cantidad de un equipo que requiere Identificador o posee Garantía.\nSi el cliente lleva varios, escanee el producto nuevamente.",
        ...
        int opcion = JOptionPane.showConfirmDialog(this,
                new Object[] { "Nueva Cantidad (Stock Disponible: " + stockMaximo + "):", txtNuevaCant },
                "Modificar Cantidad", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
        ...
        if (opcion == JOptionPane.OK_OPTION && !txtNuevaCant.getText().isEmpty()) {
            try {
                ...
                if (nuevaCant > stockMaximo) {
                    JOptionPane.showMessageDialog(this,
                            "No puede facturar " + nuevaCant + " unidades. Solo hay " + stockMaximo + " en stock.",
    ...
    private void quitarProducto() {
        ...
        if (f < 0)
            return;
        modeloTablaVentas.removeRow(f);
        recalcularTotales();
    }

    private void procesarVenta() {
        if (modeloTablaVentas.getRowCount() == 0) {
            JOptionPane.showMessageDialog(this, "La venta está vacía.", "Aviso", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        if (CCActiva == null) {
            JOptionPane.showMessageDialog(this,
                    "Operación denegada: El turno de caja no está abierto. Abra la caja antes de registrar transacciones.",
                    "Caja Cerrada", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        if (pnlCamposExtraPago.isVisible()) {
            if (cmbBanco.getSelectedIndex() == 0) {
                JOptionPane.showMessageDialog(this, "Debe seleccionar el Banco Emisor para autorizar esta transacción.",
                        "Datos Incompletos", JOptionPane.WARNING_MESSAGE);
                return;
            }
            if (txtReferenciaPago.getText().trim().isEmpty()) {
                JOptionPane.showMessageDialog(this, "Ingrese el número de Referencia, Voucher o ACH del pago.",
                        "Datos Incompletos", JOptionPane.WARNING_MESSAGE);
                return;
            }
        ...
        if (esApartado) {
            ...
            txtAbonoInicial.addKeyListener(new java.awt.event.KeyAdapter() {
                public void keyTyped(java.awt.event.KeyEvent evt) {
                    ...
                    if (!Character.isDigit(c) && c != '.')
                        evt.consume();
                    if (c == '.' && txtAbonoInicial.getText().contains("."))
                        evt.consume();
                }
            });
            ...
            int opApartado = JOptionPane.showConfirmDialog(this, fields, "Detalles del Apartado",
                    JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
            if (opApartado != JOptionPane.OK_OPTION)
                return;
            ...
            try {
                ...
                if (abono < 0 || abono > granTotal) {
                    JOptionPane.showMessageDialog(this, "El abono inicial debe ser menor o igual al total.", "Error",
                            JOptionPane.ERROR_MESSAGE);
                    return;
                }
                if (dias <= 0) {
                    JOptionPane.showMessageDialog(this, "El plazo en días debe ser mayor a 0.", "Error",
                            JOptionPane.ERROR_MESSAGE);
                    return;
                }
                ...
                int opSign = JOptionPane.showConfirmDialog(this,
                        new Object[] { "Ingrese su contraseña para firmar el apartado:", pfPass }, "Firma Requerida",
                        JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
                ...
                for (int i = 0; i < modeloTablaVentas.getRowCount(); i++) {
                    ...
                    d.setIdentificadorSerie(
                            modeloTablaVentas.getValueAt(i, 9) != null ? modeloTablaVentas.getValueAt(i, 9).toString()
                                    : null);
                    detallesA.add(d);

                    detallesPDF.add(new Object[] {
                            modeloTablaVentas.getValueAt(i, 0), modeloTablaVentas.getValueAt(i, 8),
                            modeloTablaVentas.getValueAt(i, 2),
                            modeloTablaVentas.getValueAt(i, 3), modeloTablaVentas.getValueAt(i, 4),
                            modeloTablaVentas.getValueAt(i, 5),
                ...
                if (apDAO.registrarApartado(a, detallesA)) {
                    ...
                    if (chooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
                        ...
                        try {
                            String fechaActual = new java.text.SimpleDateFormat("dd/MM/yyyy HH:mm")
                            ...
                            utilidades.GeneradorTickets.generarTicketApartadoPDF(
                                    ...
                                    a.getSaldoPendiente(),
                                    pago.nombre);
                            JOptionPane.showMessageDialog(this, "Apartado registrado con éxito.", "Éxito",
                                    JOptionPane.INFORMATION_MESSAGE);
                            if (Desktop.isDesktopSupported())
                                Desktop.getDesktop().open(archivoDestino);
                        } catch (Exception ex) {
                            JOptionPane.showMessageDialog(this,
                                    "El apartado se guardó, pero hubo un error al generar el PDF:\n" + ex.getMessage(),
                                    "Aviso", JOptionPane.WARNING_MESSAGE);
                        }
                    }
                    ...
                    JOptionPane.showMessageDialog(this, "Error crítico al guardar el apartado en la base de datos.",
                            "Error", JOptionPane.ERROR_MESSAGE);
                }

            } catch (NumberFormatException ex) {
                JOptionPane.showMessageDialog(this, "Verifique los valores numéricos ingresados.", "Error",
                        JOptionPane.ERROR_MESSAGE);
            }
            return;
        }
        ...
        JPasswordField pfPass = new JPasswordField();
        int opcion = JOptionPane.showConfirmDialog(this,
                new Object[] { "Ingrese su contraseña de cajero para autorizar la venta:", pfPass }, "Autorizar Cobro",
                JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
        ...
        if (dao.procesarVentaCompleta(idClienteActual, idUsuarioAutorizado, pago.id, sumSubtotal, sumImpuesto,
                granTotal, refPago, bancoSeleccionado, detalles)) {
            ...
            if (chooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                    ...
                    JOptionPane.showMessageDialog(this, "Venta registrada y comprobante generado con éxito.", "Éxito",
                            JOptionPane.INFORMATION_MESSAGE);
                    if (Desktop.isDesktopSupported())
                        Desktop.getDesktop().open(archivoDestino);
                } catch (Exception ex) {
                    JOptionPane.showMessageDialog(this,
                            "La venta se guardó, pero hubo un error al generar el PDF:\n" + ex.getMessage(), "Aviso",
                            JOptionPane.WARNING_MESSAGE);
                }
            }
            ...
            JOptionPane.showMessageDialog(this, "Error crítico al guardar la venta en base de datos.", "Error",
                    JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private class ItemPago {
        ...
        public ItemPago(int id, String nombre) {
            this.id = id;
            this.nombre = nombre;
        }
    ...
    private class DialogoBuscarClientePOS extends JDialog {
        public DialogoBuscarClientePOS(Frame parent) {
            ...
            tab.addMouseListener(new java.awt.event.MouseAdapter() {
                public void mouseClicked(java.awt.event.MouseEvent e) {
                    if (e.getClickCount() == 2) {
                        ...
                        idClienteActual = (int) mod.getValueAt(r, 0);
                        lblClienteSeleccionado.setText((String) mod.getValueAt(r, 2));
                        dispose();
                    }
                }
            });
            ...
            for (Cliente c : new ClienteDAO().listarClientesActivos()) {
                ...
                mod.addRow(new Object[] { c.getIdCliente(), inicial, nombreComp, c.getIdentidadCliente(),
    ...
    private class DialogoBuscarProductoPOS extends JDialog {
        public DialogoBuscarProductoPOS(Frame parent) {
            ...
            for (Producto p : lista) {
                // FILTRO ESTRICTO: Si está eliminado lógicamente (eliminado_producto == 1), NO
                // se añade a la lista
                // Nota: Si tu método en el modelo se llama diferente (ej:
                // isEliminadoProducto()), adáptalo aquí
                mod.addRow(new Object[] { p.getIdProducto(), p.getRutaImagen(), p.getCodigoBarras(),
                        p.getNombreProducto(), String.format("L %,.2f", p.getPrecioVenta()), p.getStockProducto() });
            }
            ...
            tab.addMouseListener(new java.awt.event.MouseAdapter() {
                public void mouseClicked(java.awt.event.MouseEvent e) {
                    if (e.getClickCount() == 2) {
                        ...
                        if (stockActual <= 0) {
                            JOptionPane.showMessageDialog(DialogoBuscarProductoPOS.this,
                        ...
                        Producto pSelec = lista.stream().filter(p -> p.getIdProducto() == idSelec).findFirst()
                                .orElse(null);
                        if (pSelec != null)
                            agregarProductoAVenta(pSelec);
                        dispose();
                    }
                }
            });
    ...
    private void mostrarZoomImagen(String ruta) {
        ...
        if (anchoOriginal <= 0 || altoOriginal <= 0)
            return;
        int nuevoAncho = tamano - 40;
        int nuevoAlto = tamano - 40;
        if (anchoOriginal > altoOriginal)
            nuevoAlto = (altoOriginal * nuevoAncho) / anchoOriginal;
        else
            nuevoAncho = (anchoOriginal * nuevoAlto) / altoOriginal;

        java.awt.image.BufferedImage scratch = new java.awt.image.BufferedImage(anchoOriginal, altoOriginal,
        ...
        while (w > nuevoAncho * 2 || h > nuevoAlto * 2) {
            w = (w > nuevoAncho * 2) ? w / 2 : nuevoAncho;
            h = (h > nuevoAlto * 2) ? h / 2 : nuevoAlto;
        ...
        java.awt.image.BufferedImage imgFinal = new java.awt.image.BufferedImage(nuevoAncho, nuevoAlto,
                java.awt.image.BufferedImage.TYPE_INT_ARGB);
        g2 = imgFinal.createGraphics();
        g2.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BICUBIC);
        g2.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
        g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        g2.drawImage(scratch, 0, 0, nuevoAncho, nuevoAlto, null);
        g2.dispose();

        JLabel lblZoom = new JLabel(new ImageIcon(imgFinal));
