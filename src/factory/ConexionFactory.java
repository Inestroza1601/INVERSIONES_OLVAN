...
public class ConexionFactory {
    ...
    static {
        ...
        // Apuntamos al archivo físico en la carpeta donde se está ejecutando el
        // programa
    ...
    public Connection getConexion() throws SQLException {
        ...
        try {
            ...
            if (huboFalloConexion) {
                JOptionPane.showMessageDialog(null, "Conexión Restablecida exitosamente con el servidor.",
    ...
    /**
     * TRADUCTOR DE ERRORES: Analiza el mensaje de SQL Server y muestra una alerta
     * amigable.
     */
    private void analizarErrorSQL(SQLException e) {
        ...
        if (tiempoActual - tiempoUltimoError > 10000) {
            ...
            if (msjOriginal.contains("tcp/ip") || msjOriginal.contains("connection refused")
                    || msjOriginal.contains("se ha denegado la conexión")) {
                ...
                msjTraducido = "Las credenciales son incorrectas.\n\n"
                        + "Verifique que el usuario '" + usuario
                        + "' y la contraseña sean correctos en el archivo config.properties.";

            } else if (msjOriginal.contains("database")
                    && (msjOriginal.contains("not found") || msjOriginal.contains("no existe"))) {
